...
Heavy Task
เพราะมีการสร้าง ตัวแปรภายในฟังก์ชัน (local variables) ที่มีขนาดใหญ่หลายตัว เช่น

2. การใช้ heap แทน stack มีข้อดีอย่างไร?
การใช้ heap memory (malloc/free) แทนการประกาศตัวแปรใน stack มีข้อดีคือ

✅ ลดการใช้พื้นที่ stack ของแต่ละ task

✅ สามารถจัดสรรหน่วยความจำขนาดใหญ่ได้ เพราะ heap มีขนาดรวมมากกว่า stack

✅ ยืดหยุ่นกว่า — สามารถจองและคืนหน่วยความจำได้ตามต้องการระหว่างทำงาน

⚠️ แต่ต้องระวังเรื่อง memory leak หากจองแล้วไม่ free

🧠 สรุป: การใช้ heap ช่วยลดปัญหา stack overflow และเหมาะกับข้อมูลขนาดใหญ่

3. Stack overflow เกิดขึ้นเมื่อไหร่และทำอย่างไรป้องกัน?
Stack overflow เกิดขึ้นเมื่อ task ใช้พื้นที่ stack เกินขนาดที่กำหนดไว้ เช่น

ใช้ local array ขนาดใหญ่

มี recursion ลึกมาก

หรือเรียกฟังก์ชันซ้อนกันหลายชั้น

วิธีป้องกัน:

🧮 คำนวณ stack size ให้เหมาะสมกับการใช้งานของแต่ละ task

🧠 ใช้ uxTaskGetStackHighWaterMark() เพื่อตรวจสอบการใช้ stack จริง

⚙️ เปิดการตรวจสอบใน FreeRTOS:

CONFIG_FREERTOS_CHECK_STACKOVERFLOW=2


💾 ใช้ heap สำหรับข้อมูลใหญ่แทน stack

🚫 หลีกเลี่ยง recursion ที่ลึกเกินไป

🧠 สรุป: Overflow เกิดจากการใช้ stack เกินขนาด ป้องกันได้ด้วยการ monitor และจัด stack size ให้เหมาะสม

4. การตั้งค่า stack size ควรพิจารณาจากอะไร?

การกำหนด stack size ของแต่ละ task ควรพิจารณาจาก:

พฤติกรรมของ task – ถ้ามี local variables หรือฟังก์ชันซ้อนหลายชั้น ควรให้ stack เยอะกว่า

การใช้ฟังก์ชันไลบรารี – ฟังก์ชันบางตัว (เช่น printf, sprintf) ใช้ stack เยอะ

ผลจากการ monitor จริง – ใช้ค่า uxTaskGetStackHighWaterMark() ดูว่าควรเพิ่มหรือลด

ประเภทของข้อมูล – ข้อมูลขนาดใหญ่ควรเก็บใน heap

จำนวน task รวมทั้งหมด – เพราะ stack ทุก task รวมกันต้องไม่เกินหน่วยความจำทั้งหมดของระบบ

🧠 สรุป: ตั้ง stack size ตามลักษณะการทำงานและผลการ monitor ของแต่ละ task เพื่อใช้หน่วยความจำอย่างมีประสิทธิภาพ

5. Recursion ส่งผลต่อ stack usage อย่างไร?

Recursion ใช้ stack เพิ่มขึ้นตามระดับการเรียกซ้ำ อาจเกิด overflow ถ้าลึกเกินไป
